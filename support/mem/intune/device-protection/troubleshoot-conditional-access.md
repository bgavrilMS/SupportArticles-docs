...
- Desktop applications must use modern authentication mechanisms, which rely on users interacting with a browser or with an authentication broker. Legacy authentication mechanisms such as apps sending the [password]([url](https://learn.microsoft.com/en-us/azure/active-directory/develop/v2-oauth-ropc)) on their own are not supported.
